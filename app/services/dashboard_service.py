...
def get_kpis(start_date: Optional[datetime] = None, end_date: Optional[datetime] = None, 
             cartera: Optional[str] = None, gestor_id: Optional[int] = None) -> Dict:
    """
...
def get_performance_chart_data(start_date: Optional[datetime] = None, end_date: Optional[datetime] = None,
                               cartera: Optional[str] = None) -> Dict:
    """
...
def get_cartera_distribution() -> Dict:
    ...
    result = db.session.query(
        Case.cartera,
        func.sum(Case.amount).label('total')
    ).group_by(Case.cartera).all()
...
def get_gestores_ranking(limit: int = 10) -> List[Dict]:
    ...
    for gestor in gestores:
        ...
        if case_ids:
            ...
            fulfilled_promises = Promise.query.filter(
                Promise.case_id.in_(case_ids),
...
def get_comparison_data() -> Dict:
    ...
    # Mes actual
    current_cases = Case.query.filter(
        Case.created_at >= current_month_start,
        Case.status == 'pagada'
    ).all()
    current_monto = sum(float(c.amount) for c in current_cases)
    
    current_promises = Promise.query.filter(
        Promise.created_at >= current_month_start
    ).all()
    ...
    current_activities = Activity.query.filter(
        Activity.created_at >= current_month_start
    ).count()
    
    # Mes anterior
    previous_cases = Case.query.filter(
        Case.created_at >= previous_month_start,
        Case.created_at < previous_month_end,
        Case.status == 'pagada'
    ).all()
    previous_monto = sum(float(c.amount) for c in previous_cases)
    
    previous_promises = Promise.query.filter(
        Promise.created_at >= previous_month_start,
        Promise.created_at < previous_month_end
    ).all()
    ...
    previous_activities = Activity.query.filter(
        Activity.created_at >= previous_month_start,
        Activity.created_at < previous_month_end
    ).count()
