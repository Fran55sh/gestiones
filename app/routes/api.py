...
def get_case(case_id):
    ...
    try:
        ...
        if user_role == 'gestor' and case.assigned_to_id != user_id:
            ...
            return jsonify({
                'success': False,
                'error': 'No tiene permisos para ver este caso'
            }), 403
...
def update_status():
    ...
    try:
        ...
        if not case_id:
            app.logger.warning(f"Update status sin case_id - solo actualización UI. Status: {status}, Form data: {dict(request.form)}")
        ...
        if not case_id:
            ...
            return jsonify({
                'success': True,
                'message': 'Estado actualizado (solo UI)'
            })
        ...
        if user_role != 'admin' and case.assigned_to_id != user_id:
            return jsonify({
                'success': False,
                'error': 'No tiene permisos para actualizar este caso'
            }), 403
        ...
        app.logger.info(f"Estado actualizado: Caso {case_id} de '{old_status}'/'{old_management_status}' a '{new_status}'/'{status}'")
...
def get_gestor_cases():
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'data': [c.to_dict(include_relations=True) for c in cases]
        })
    except Exception as e:
...
def register_management():
    ...
    try:
        ...
        if user_role != 'admin' and case.assigned_to_id != user_id:
            return jsonify({
                'success': False,
                'error': 'No tiene permisos para este caso'
            }), 403
