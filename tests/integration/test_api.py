...
def test_create_case(client, authenticated_client):
    ...
    response = authenticated_client.post("/api/cases", json=case_data, content_type="application/json")
...
def test_update_case(client, authenticated_client, test_case):
    ...
    update_data = {"status": "pagada", "notes": "Updated notes"}
    response = authenticated_client.put(f"/api/cases/{test_case.id}", json=update_data, content_type="application/json")
    assert response.status_code == 200
    data = response.get_json()
    assert data["success"] is True
...
def test_create_promise(client, authenticated_client, test_case):
    ...
    promise_data = {"amount": 2000.00, "promise_date": date.today().isoformat(), "status": "pending"}
    response = authenticated_client.post(
        f"/api/cases/{test_case.id}/promises", json=promise_data, content_type="application/json"
    )
    assert response.status_code == 201
    data = response.get_json()
    assert data["success"] is True
...
def test_create_activity(client, authenticated_client, test_case):
    ...
    activity_data = {"type": "call", "notes": "Test call activity"}
    response = authenticated_client.post(
        f"/api/cases/{test_case.id}/activities", json=activity_data, content_type="application/json"
    )
    assert response.status_code == 201
    data = response.get_json()
    assert data["success"] is True
...
def test_api_requires_auth(client):
    ...
    endpoints = ["/api/dashboard/kpis", "/api/dashboard/charts/performance", "/api/cases"]
