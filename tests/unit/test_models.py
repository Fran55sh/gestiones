...
def test_promise_creation(app, sample_case):
    ...
    promise = Promise(case_id=sample_case.id, amount=Decimal("5000.00"), promise_date=date.today(), status="pending")
...
def test_activity_creation(app, sample_case, sample_user):
    ...
    activity = Activity(case_id=sample_case.id, type="call", notes="Llamada realizada", created_by_id=sample_user.id)
...
def test_contact_submission_creation(app):
    ...
    submission = ContactSubmission(
        entity="Test Company", name="Jane Doe", email="jane@test.com", phone="1234567890", message="Test message"
    )
